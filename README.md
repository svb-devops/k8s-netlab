...
本项目使用**滑动窗口内存速率限制**（`backend/rate_limiter.py`）：

| 端点 | 限制 | 窗口 |
|------|------|------|
| `POST /api/login` | 10 次 | 60 秒 |
| `POST /api/vms` | 5 次 | 60 秒 |
...
限制:
  ⚠️ 仅限单进程内共享状态
  ⚠️ 重启后计数器清零
  ⚠️ 多 worker / 多实例环境下不一致
```

### 适用场景

当前配置适合：**单实例部署，10–50 并发用户**（教学实验环境标准配置）。

### 未来升级路径

若需支持更大规模（50+ 用户、多实例），可升级为 Redis：

```bash
pip install redis
# 将 rate_limiter.py 替换为 redis-py 滑动窗口实现
# 预估工作量：2–3 小时
```
