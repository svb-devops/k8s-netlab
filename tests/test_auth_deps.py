...
class TestGetCurrentUser:
    ...
    async def test_no_token_raises_401(self):
        """session_token 为 None 时必须抛 401（未登录）。"""
    ...
    async def test_invalid_session_raises_401(self):
        ...
        with patch("backend.auth_deps.auth_manager") as mock_mgr:
            ...
            with pytest.raises(HTTPException) as exc_info:
                await get_current_user(session_token="expired-token")
        assert exc_info.value.status_code == 401
    ...
    async def test_valid_session_returns_username(self):
        ...
        with patch("backend.auth_deps.auth_manager") as mock_mgr:
            ...
            result = await get_current_user(session_token="valid-token")
        assert result == "alice"
...
class TestGetCurrentUserOptional:
    ...
    async def test_no_token_returns_none(self):
        """session_token 为 None 时必须返回 None，不抛异常（可选认证端点回归）。"""
        from backend.auth_deps import get_current_user_optional

        result = await get_current_user_optional(session_token=None)
    ...
    async def test_valid_session_returns_username(self):
        ...
        with patch("backend.auth_deps.auth_manager") as mock_mgr:
            ...
            result = await get_current_user_optional(session_token="valid-token")
        assert result == "bob"
