...
> **Current state**: ALL 6 ops tickets VERIFIED; SMALL_CUSTOMER_PILOT_BLOCKED (no suitable customer — technical system ready); **Internal Rehearsal to Publish Candidate COMPLETE — PUBLISH_CANDIDATE_BLOCKED_BY_MISSING_REHEARSAL_BRIDGE**: Full pipeline Article→Contract→AdminReview→Validator→LabDraft(DRAFT) passed; BLOCKER-001: `LabSessionService.run_precheck()` requires publish_status=PUBLISHED; converted LabDraft is always DRAFT; publishing would add to learner catalog (forbidden invariant); 0 LLM calls; primary slogan updated to "读了能练，练完即熟"; recommended next step: Internal Rehearsal Bridge Implementation; see `docs/labgen/K8S_ARTICLE_TO_LAB_INTERNAL_REHEARSAL_TO_PUBLISH_CANDIDATE_RESULT_v0.1.md`  
