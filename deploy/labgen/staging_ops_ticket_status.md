...
> **Current state**: ALL 6 ops tickets VERIFIED; SMALL_CUSTOMER_PILOT_BLOCKED (no suitable customer — technical system ready); **Internal Rehearsal Bridge READY_WITH_NOTES** (G-30): `POST /internal/rehearsal-sessions` implemented; dual-auth (X-Admin-Token + admin_user); session_type=INTERNAL_REHEARSAL; full catalog isolation; 2 HIGH + 2 MEDIUM safety-reviewer findings all fixed; 3668 tests; 93.27% coverage; next: admin executes rehearsal against ConfigMap LabDraft, then publish; see `docs/labgen/ARTICLE_TO_LAB_INTERNAL_REHEARSAL_BRIDGE_RESULT_v0.1.md`  
