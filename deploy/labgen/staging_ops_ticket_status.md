...
> **Updated**: 2026-06-24 — **G-60 Article-Lab Content Consistency Remediation ARTICLE_LAB_CONTENT_CONSISTENCY_REMEDIATED_WITH_NOTES**: Linux article (Directus id=3, slug=linux-files-permissions-basics) created and bound to Linux lab 6c439064; K8s welcome article unbound; topic_consistency.py rule-based gate added; CTA topic_consistency_warning=None; StaticValidator content.article_lab_topic_consistency check (draft_warning); Step 3 data confirmed internally consistent (G-59 result doc was incorrect); Reader E2E g60-reader-01 session 3254de61 PASS (LAB_CLOSED cleanup_verified=True residual=0); 12 negative checks PASS; 33 new tests; 4418 tests PASS 92.28% coverage; BLOCKER=0 HIGH=0 MEDIUM=0; article_type=mock_admin_article (not pre-existing externally published); no live LLM/URL scraping/public upload/customer pilot; VMID 500-599 untouched; artifact: ARTICLE_LAB_CONTENT_CONSISTENCY_REMEDIATION_RESULT_v0.1.md; **G-59 Admin Article Input MVP End-to-End Validation ADMIN_ARTICLE_INPUT_MVP_E2E_READY_WITH_NOTES**: 绑定真实 Directus 文章至 Linux lab 6c439064；admin PATCH article metadata persisted；GET /api/labgen/drafts/{id}/cta 返回 4 种 CTA 格式（系统生成）；Reader g59-reader-01 完整 E2E 通过（Start→4 steps→LAB_CLOSED cleanup_verified=True residual=0）；13 negative checks PASS；4385 tests PASS 0 failures；服务重启加载 G-58 代码；BLOCKER=0 HIGH=0 MEDIUM=1(chmod content inconsistency NOTE)；health check PASS；source_article_id 不泄露；no public upload/live LLM/customer pilot；VMID 500-599 untouched；artifact: ADMIN_ARTICLE_INPUT_MVP_E2E_RESULT_v0.1.md；推荐下一步：Article Detail Embedded CTA Component; **G-58 Article URL + CTA Tool Implementation ARTICLE_URL_AND_CTA_TOOL_READY**: LabDraft 加 5 字段 + 3 validators；GET /api/labgen/drafts/{id}/cta admin-only；42 new tests 92.26% coverage; **G-55 Second Linux Trusted Reader Pilot Planning Review SECOND_LINUX_TRUSTED_READER_PILOT_PLANNING_REVIEW_READY_WITH_NOTES**: plan reviewed and updated (reader identity format + test window format + independence tracking added); health check PASS (catalog=6, active=0, tainted={}, residual=0, LLM mode=fake_only, VMID 500-599 untouched, service healthy, K8s 27 targeted tests PASS); 4335 tests 92.26% coverage; BLOCKER=0 HIGH=0 MEDIUM=0; second reader NOT started; account NOT created; cohort BLOCKED; pilot awaiting user: reader identity + test window + explicit YES approval ("YES, approve Second Linux Trusted Reader Pilot Execution."); artifacts: SECOND_LINUX_TRUSTED_READER_PILOT_PLANNING_REVIEW_RESULT_v0.1.md + updated SECOND_LINUX_TRUSTED_READER_PILOT_PLAN_v0.1.md; ALL 6 tickets VERIFIED; **G-54 Linux Pilot Feedback Attestation, Environment Shutdown & Next Reader Planning LINUX_PILOT_FEEDBACK_ATTESTATION_AND_SHUTDOWN_READY_WITH_NOTES**: owner onsite attestation recorded (USER_ONSITE_ATTESTATION: "测试非常顺利"); MEDIUM-001 CLOSED→NOTE (owner-attested, not complete 10-Q); MEDIUM-002 reclassified LOW (USER_OBSERVED_SMOOTH_COMPLETION, not fully instrumented); session 8d9bd8db LAB_CLOSED cleanup_verified=True workspace ABSENT; active=0 tainted={} health OK LLM=0 VMID 500-599 untouched; K8s 211 targeted tests PASS; second reader planning doc created (SECOND_LINUX_TRUSTED_READER_PILOT_PLAN_v0.1.md); second reader NOT started; cohort BLOCKED (need second pass + 10-Q); artifacts: LINUX_PILOT_FEEDBACK_ATTESTATION_v0.1.md + LINUX_PILOT_FEEDBACK_ATTESTATION_AND_SHUTDOWN_RESULT_v0.1.md + SECOND_LINUX_TRUSTED_READER_PILOT_PLAN_v0.1.md; **G-53 Linux Pilot Feedback Remediation LINUX_PILOT_FEEDBACK_REMEDIATION_NEEDS_ITERATION**: MEDIUM-001 NOT CLOSED (10-Q feedback not obtainable by Claude Code — no feedback mechanism in system, not provided in task submission); MEDIUM-002 NOT CLOSED (69-second session unconfirmable as independent — no step timestamps, no retry/help records; classified TECHNICAL_SMOKE_BY_TRUSTED_READER not INDEPENDENT_COMPLETION); expansion BLOCKED; K8s 211 tests PASS; active=0, tainted={}, health OK, LLM=0, VMID 500-599 untouched; no second reader/cohort; recommended: Hold Expansion (Path A: user provides 10-Q from memory; Path B: Second Linux Trusted Reader Pilot with built-in feedback capture); artifact: LINUX_PILOT_FEEDBACK_REMEDIATION_RESULT_v0.1.md; **G-52 Linux Trusted Reader Pilot Exit Review LINUX_TRUSTED_READER_PILOT_EXIT_REVIEW_READY_WITH_NOTES**: technical pass confirmed (session 8d9bd8db LAB_CLOSED cleanup_verified=True 4 steps, residual=0, tainted={}, health OK, K8s 211 targeted tests PASS, LLM=0, VMID 500-599 untouched); BLOCKER=0, HIGH=0; MEDIUM-001 (qualitative feedback not recorded — blocks expansion); MEDIUM-002 (independence evidence thin, 69-second session); no BLOCKER/HIGH; not public launch; not customer pilot; recommended next step: Linux Pilot Feedback Remediation; artifact: LINUX_TRUSTED_READER_PILOT_EXIT_REVIEW_RESULT_v0.1.md; **G-51 Linux Trusted Reader Pilot LINUX_TRUSTED_READER_PILOT_PASSED**: real reader `lab_test` completed full Linux lab flow; session `8d9bd8db` LAB_CLOSED cleanup_verified=True 4 steps complete; post-run audit clean (active=0, tainted={}, health OK, error logs empty, VMID 500-599 untouched, K8s Lab 5 CLEAN, LLM=0); VM 401 missing (Linux pilot unaffected — local workspaces only); see `docs/labgen/LINUX_TRUSTED_READER_PILOT_RESULT_v0.1.md`; **G-50 BLOCKER FIX: Linux terminal credential_error resolved** — `lab_kubectl_ws.py` now routes Linux sessions (`vm_id='linux-sandbox'`) to `_handle_linux_terminal()` (LinuxRuntimeAdapter + local `>` redirection); 24 regression tests; 4335 tests 92.26% coverage; service restarted; test steps doc updated (printf→echo); **G-50 Linux Pilot Owner Rehearsal Prep LINUX_PILOT_OWNER_REHEARSAL_READY_FOR_USER**: VM 401 started (STOPPED→running), VM tracker pilot_reader→lnx-rehearsal-01, account lnx-rehearsal-01 created, service restarted (G-48 code loaded: experiment_background+check_count), negative checks 8/8 PASS, K8s CLEAN, catalog=6, active sessions=0, tainted=0, LLM=0, VMID 500-599 untouched; owner rehearsal account/steps ready; real pilot NOT started; awaiting owner rehearsal completion + user YES; artifacts: LINUX_OWNER_REHEARSAL_TEST_STEPS_v0.1.md + LINUX_PILOT_OWNER_REHEARSAL_RESULT_v0.1.md; **G-49 Linux Trusted Reader Pilot Planning LINUX_TRUSTED_READER_PILOT_PLANNING_READY_WITH_NOTES**: planning complete, health check PASS with ops notes (VM 401 stopped, tracker reassignment needed, no code blockers); BLOCKER=0 HIGH=0 MEDIUM=0; LOW-002(article.html no embedded CTA, accepted); approval_status=PENDING_USER_APPROVAL; pilot NOT started; no code changes; LLM=0; VMID 500-599 untouched; catalog=6; K8s CLEAN; 4311 tests baseline unchanged; artifacts: PILOT_PLAN + APPROVAL_GATE + PLANNING_RESULT; see `docs/labgen/LINUX_TRUSTED_READER_PILOT_PLANNING_RESULT_v0.1.md`; G-48 Linux Content Quality Iteration LINUX_CONTENT_QUALITY_ITERATION_READY; Runtime Session Smoke PASSED_WITH_NOTES; Pilot Gate READY_WITH_NOTES; First Pilot Lab RELEASED; First Pilot User ONBOARDED_WITH_NOTES; Pilot Feedback TRIAGED; Frontend Learner Smoke PASSED_WITH_NOTES; Second Pilot User ONBOARDED; Ops Runbook HARDENED; Third Pilot User ONBOARDED; Second Pilot Lab READY; Fourth Pilot User Second Lab ONBOARDED; Second Lab Feedback Triage TRIAGED_WITH_ITERATION; Fifth Pilot User Second Lab ONBOARDED; Third Pilot Lab READY; Sixth Pilot User Third Lab ONBOARDED; Deployment Lab READY; Seventh Pilot User Deployment Lab ONBOARDED; Deployment Feedback Triage TRIAGED_WITH_ITERATION; Eighth Pilot User Deployment Lab ONBOARDED; Small Cohort Readiness Gate SMALL_COHORT_READY_WITH_NOTES; Small Cohort Pilot SMALL_COHORT_PILOT_COMPLETED_WITH_NOTES; Small Cohort Triage SMALL_COHORT_TRIAGED_NEEDS_ITERATION; Real Human Learner Validation REAL_HUMAN_LEARNER_VALIDATED_WITH_NOTES; Learner kubectl Terminal Integration LEARNER_KUBECTL_TERMINAL_READY; Terminal Post-Integration Runtime Hardening TERMINAL_RUNTIME_HARDENED_WITH_NOTES; Real Human Re-validation for Labs 2-4 REAL_HUMAN_LABS_2_4_VALIDATED_WITH_NOTES; Real Human Cohort Round 2 REAL_HUMAN_COHORT_ROUND2_COMPLETED_WITH_NOTES; Small Customer Pilot Preparation Gate SMALL_CUSTOMER_PILOT_PREP_READY_WITH_NOTES; Small Customer Pilot Execution SMALL_CUSTOMER_PILOT_BLOCKED; Article-to-Lab Pipeline Design Gate ARTICLE_TO_LAB_PIPELINE_DESIGN_READY_WITH_NOTES; Article-to-Lab Implementation Prerequisites ARTICLE_TO_LAB_PREREQS_READY_WITH_NOTES; Article-to-Lab MVP Contract Schema Gate ARTICLE_TO_LAB_SCHEMA_READY_WITH_NOTES; Article-to-Lab Draft Mode Implementation K8S_ARTICLE_TO_LAB_DRAFT_MODE_READY_WITH_NOTES; Admin Review Rehearsal K8S_ARTICLE_TO_LAB_ADMIN_REHEARSAL_PASSED_WITH_NOTES; North Star Re-Alignment NORTH_STAR_REALIGNED_WITH_NOTES; **Internal Rehearsal to Publish Candidate PUBLISH_CANDIDATE_BLOCKED_BY_MISSING_REHEARSAL_BRIDGE**; **Linux Runtime Adapter Spike LINUX_RUNTIME_ADAPTER_SPIKE_READY_WITH_NOTES** (G-37); **Linux Verifier Adapter Spike LINUX_VERIFIER_ADAPTER_SPIKE_READY_WITH_NOTES** (G-38); **Linux Verifier Adapter Hardening LINUX_VERIFIER_ADAPTER_HARDENED_WITH_NOTES** (G-39); **Linux Guided Practice Draft Template LINUX_GUIDED_PRACTICE_DRAFT_TEMPLATE_READY_WITH_NOTES** (G-40); **Linux Internal Rehearsal Bridge LINUX_INTERNAL_REHEARSAL_BRIDGE_READY_WITH_NOTES** (G-41); **Linux E2E Internal Rehearsal Acceptance LINUX_E2E_INTERNAL_REHEARSAL_ACCEPTANCE_PASSED_WITH_NOTES** (G-42); **Linux Publish Candidate Dry Run LINUX_PUBLISH_CANDIDATE_DRY_RUN_READY** (G-43); **Linux Article-linked Lab Publish Gate LINUX_ARTICLE_LINKED_LAB_PUBLISHED_WITH_NOTES** (G-44); **Linux Learner Runtime Enablement Gate LINUX_LEARNER_RUNTIME_ENABLEMENT_GATE_PASSED** (G-45); **Linux Reader-facing CTA Dry Run LINUX_READER_FACING_CTA_DRY_RUN_READY_WITH_NOTES** (G-47): operator-controlled learner (linux-cta-dry-run, session fc45a40e) mock CTA→deep link→lab detail landing→Start LAB_ACTIVE→steps 1-4 PASS→Complete LAB_CLOSED cleanup_verified=True residual=0; negative checks 11/11 PASS; AI tutor context present, LLM=0; post-run audit clean; MEDIUM-001(experiment_background empty) MEDIUM-002(troubleshoot empty) LOW-001(completion_summary empty) LOW-002(no embedded CTA in article.html); NOT Trusted Reader Pilot; NOT public launch; K8s Lab 5 CLEAN; catalog=6; see `docs/labgen/LINUX_READER_FACING_CTA_DRY_RUN_RESULT_v0.1.md`; **Linux Content Quality Iteration LINUX_CONTENT_QUALITY_ITERATION_READY** (G-48): Step.troubleshoot + LabDraft.experiment_background + completion_summary schema fields added (optional, backward compat); LearnerLabDetail exposes new fields; LearnerSessionStepStatus.step_troubleshoot for current step; check_count bug fixed (linux_verify count now included); Linux lab content authored (4 troubleshoot hints + background + completion_summary, no TODOs); frontend: Background card + "Need help?" collapsible hint; 25 new tests; 4311 tests, 92.49% coverage; MEDIUM-001/002 CLOSED; LOW-001 CLOSED; LOW-003 CLOSED; NOT Trusted Reader Pilot; K8s CLEAN; VMID 500-599 untouched; **Linux Learner Runtime E2E Smoke Acceptance LINUX_LEARNER_RUNTIME_E2E_SMOKE_PASSED_WITH_NOTES** (G-46)  
> **Current state**: ALL 6 ops tickets VERIFIED; SMALL_CUSTOMER_PILOT_BLOCKED (no suitable customer — technical system ready); Guided Practice Quality Iteration GUIDED_PRACTICE_QUALITY_READY_FOR_TRUSTED_READER (G-33); **K8s Trusted Reader Pilot PASSED** (G-34): session a301676a LAB_CLOSED cleanup_verified=True step_1+step_2 PASS observer-confirmed no hiccups; **Linux Domain Proof Design Gate LINUX_DOMAIN_PROOF_DESIGN_READY_WITH_NOTES** (G-35): 5 BLOCKER couplings enumerated; **Linux Domain Contract Schema Extension LINUX_DOMAIN_CONTRACT_SCHEMA_READY_WITH_NOTES** (G-36): 60 tests; publish blocked; K8s regression clean; **Linux Runtime Adapter Spike LINUX_RUNTIME_ADAPTER_SPIKE_READY_WITH_NOTES** (G-37): 88 tests; 3885 total; **Linux Verifier Adapter Spike LINUX_VERIFIER_ADAPTER_SPIKE_READY_WITH_NOTES** (G-38): 5 primitives; 48 tests; TOCTOU documented; **Linux Verifier Adapter Hardening LINUX_VERIFIER_ADAPTER_HARDENED_WITH_NOTES** (G-39): TOCTOU MEDIUM closed (lstat+recheck_containment+O_NOFOLLOW); safety-reviewer no BLOCKER; 61 hardening tests; list_files_recursive aligned; see `docs/labgen/LINUX_VERIFIER_ADAPTER_HARDENING_RESULT_v0.1.md`; **Linux Guided Practice Draft Template LINUX_GUIDED_PRACTICE_DRAFT_TEMPLATE_READY_WITH_NOTES** (G-40): LinuxFilesPermissionsTemplate (4 steps, all 5 primitives, no LLM, no placeholders); ai_tutor_context field added; generate_linux() in stub_generator; FeasibilityClassifier /etc MEDIUM fixed; StaticValidator linux.verifiers_present check added; publish blocked; 68 tests; 4065 total tests; 93.27% coverage; K8s regression CLEAN; **Linux Internal Rehearsal Bridge LINUX_INTERNAL_REHEARSAL_BRIDGE_READY_WITH_NOTES** (G-41): LinuxRehearsalService (9-point precheck, _safe_exec_command shell-redirect intercept, dual-auth routes, session_type + student_username guard); 70 new tests; 4136 total; 92.44% coverage; publish blocked; catalog zero; K8s CLEAN; LLM=0; VMID 500-599 untouched; **Linux E2E Internal Rehearsal Acceptance LINUX_E2E_INTERNAL_REHEARSAL_ACCEPTANCE_PASSED_WITH_NOTES** (G-42): real service E2E chain passed (draft→session→step1→step2→step3→complete→LAB_CLOSED cleanup_verified=True rehearsal_completed=True); abort path passed; all negative checks passed (401/403/policy_rejected); MEDIUM fix: WorkspacePathEscapeError now policy_rejected=True; NOT Trusted Reader Pilot; publish blocked; catalog 0; K8s CLEAN; 4140 tests; 92.52% coverage; see `docs/labgen/LINUX_E2E_INTERNAL_REHEARSAL_ACCEPTANCE_RESULT_v0.1.md`; **Linux Publish Candidate Dry Run LINUX_PUBLISH_CANDIDATE_DRY_RUN_READY** (G-43): PublishCandidateDryRunService (6 gates read-only); POST /api/labgen/drafts/{id}/publish-candidate-dry-run (admin-only); 2 MEDIUM fixes (env sudo token scan + normpath path traversal); 58 tests; 4198 total; 92.64% coverage; actual_publish_performed=False / learner_catalog_changed=False immutable; Linux catalog=0; K8s CLEAN; LLM=0; VMID 500-599 untouched; commit c0bc4ce; see `docs/labgen/LINUX_PUBLISH_CANDIDATE_DRY_RUN_RESULT_v0.1.md`; **Linux Article-linked Lab Publish Gate LINUX_ARTICLE_LINKED_LAB_PUBLISHED_WITH_NOTES** (G-44): gate lift (removed linux.publish_blocked_until_runtime from StaticValidator); PRECHECK_LINUX_LEARNER_NOT_SUPPORTED added; 30 new tests + 15 updated; 4228 total; 92.63% coverage; BLOCKER=0; safety-reviewer HIGH false positive + MEDIUM×2 fixed; Codex P1 design intent; 1 Linux lab published (6c439064); catalog 5→6; K8s Lab 5 CLEAN; learner Start blocked with precheck.linux_learner_not_yet_available; commit abaf7a6; see `docs/labgen/LINUX_ARTICLE_LINKED_LAB_PUBLISH_GATE_RESULT_v0.1.md`; **Linux Learner Runtime Enablement Gate LINUX_LEARNER_RUNTIME_ENABLEMENT_GATE_PASSED** (G-45): LABGEN_LINUX_LEARNER_ENABLED_LAB_IDS feature flag; singleton LinuxRuntimeAdapter; _do_cleanup sentinel dispatch (vm_id anchor); fail-closed adapter=None; 58 new tests; 4286 total; 92.48% coverage; commit 68bb085; see `docs/labgen/LINUX_LEARNER_RUNTIME_ENABLEMENT_GATE_RESULT_v0.1.md`; **Linux Learner Runtime E2E Smoke Acceptance LINUX_LEARNER_RUNTIME_E2E_SMOKE_PASSED_WITH_NOTES** (G-46): real learner smoke (linux-smoke-learner, session 0a38c7c5) Start LAB_ACTIVE→steps 1-4 PASS→Complete LAB_CLOSED cleanup_verified=True residual=0; abort path PASS; BLOCKER fixed (lfp-step-4 linux_no_residual_files removed — UX contradiction: workspace can't be empty before complete()); negative checks all PASS; post-run audit clean; .env LABGEN_LINUX_LEARNER_ENABLED_LAB_IDS=6c439064; 4286 tests; 92.48% coverage; NOT Trusted Reader Pilot; NOT public launch; LLM=0; VMID 500-599 untouched; see `docs/labgen/LINUX_LEARNER_RUNTIME_E2E_SMOKE_ACCEPTANCE_RESULT_v0.1.md`; **Linux Reader-facing CTA Dry Run LINUX_READER_FACING_CTA_DRY_RUN_READY_WITH_NOTES** (G-47): mock CTA→deep link→lab detail→Start LAB_ACTIVE→steps 1-4 PASS→Complete LAB_CLOSED cleanup_verified=True residual=0; 11 negative checks PASS; AI tutor context LLM=0; MEDIUM×2(experiment_background/troubleshoot empty) LOW×2(completion_summary/no article CTA component); NOT Trusted Reader Pilot; catalog=6; K8s CLEAN; see `docs/labgen/LINUX_READER_FACING_CTA_DRY_RUN_RESULT_v0.1.md`  
