...
> **Current state**: ALL 6 ops tickets VERIFIED; SMALL_CUSTOMER_PILOT_BLOCKED (no suitable customer — technical system ready); **Article-linked Lab Publish Gate PUBLISH_GATE_READY** (G-31): complete article→rehearsal→publish pipeline executed in staging; 5 labs in catalog (4 existing + 1 article-linked cf019133); `StepProgressionService` INTERNAL_REHEARSAL bypass; platform/verifier kubeconfig stale IP fixed; 30 tests; 3632 tests total, 93%+ coverage; see `docs/labgen/ADMIN_CURATED_ARTICLE_LINKED_LAB_PUBLISH_GATE_RESULT_v0.1.md`  
